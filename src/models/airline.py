...
class Airline(db.Model):
    ...
    alliance = db.Column(db.String(20), nullable=True)  # SkyTeam, Oneworld, Star Alliance, or None for unallianced
    loyalty_program = db.Column(db.String(100), nullable=False)
    country = db.Column(db.String(100), nullable=True)  # Airline's home country
...
class Airport(db.Model):
    ...
    state = db.Column(db.String(100), nullable=True)  # State/Province if applicable
    country = db.Column(db.String(100), nullable=False)
    latitude = db.Column(db.Float, nullable=False)
    longitude = db.Column(db.Float, nullable=False)
    timezone = db.Column(db.String(50), nullable=True)  # Timezone information
...
class LoyaltyProgram(db.Model):
    ...
    alliance = db.Column(db.String(20), nullable=True)  # Can be None for unallianced
    earning_model = db.Column(db.String(20), nullable=False, default='distance')  # 'distance' or 'revenue'
    base_earning_rate = db.Column(db.Float, nullable=False)  # Base miles per mile flown or per dollar spent
    currency = db.Column(db.String(10), nullable=True, default='USD')  # For revenue-based programs
    
    # Elite status tier names (airline-specific)
    silver_tier_name = db.Column(db.String(50), nullable=True)
    gold_tier_name = db.Column(db.String(50), nullable=True)
    platinum_tier_name = db.Column(db.String(50), nullable=True)
    
    # Elite status bonuses (as multipliers)
    silver_bonus = db.Column(db.Float, nullable=False, default=0.25)  # 25% bonus
    gold_bonus = db.Column(db.Float, nullable=False, default=0.50)    # 50% bonus
    platinum_bonus = db.Column(db.Float, nullable=False, default=1.0) # 100% bonus
...
class EarningRate(db.Model):
    ...
    minimum_miles = db.Column(db.Integer, default=500)  # Minimum miles per segment (industry standard)
...
class FareClass(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(50), nullable=False)  # Economy, Premium Economy, Business, First
...
def calculate_miles_earned(distance, earning_rate, elite_status='none', ticket_price=None, is_partner=False):
    ...
    # Note: Maximum miles rule not implemented in current schema
